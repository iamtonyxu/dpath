function y = pfir_filter(x, h, decim, gain_db, bypass)
...
%       (same orientation as x)
    ...
    if ~isvector(x)
        error('pfir_filter: input x must be a 1-D vector.');
    end

    n_taps = length(h);
    if ~ismember(n_taps, [24, 48, 72])
        error('pfir_filter: h must have 24, 48, or 72 taps (got %d).', n_taps);
    end

    if ~ismember(decim, [1, 2, 4])
        error('pfir_filter: decim must be 1, 2, or 4 (got %d).', decim);
    end

    if ~ismember(gain_db, [6, 0, -6, -12])
        error('pfir_filter: gain_db must be +6, 0, -6, or -12 dB (got %g).', gain_db);
    end
